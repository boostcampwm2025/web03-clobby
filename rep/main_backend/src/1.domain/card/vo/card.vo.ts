...
export function thumbnailPathVo( thumbnail_path : Exclude<CardProps["thumbnail_path"], undefined> ) : string {
  ...
  if ( thumbnail_path.length > 255 ) throw new NotAllowMaxLengthText({ name, length });
...
// workspace_width을 검증
export function workspaceWidthVo( workspace_width : CardProps["workspace_width"] ) : number {
  const name : string = "workspace_width";

  if ( workspace_width !== 0 ) baseVo({ name, value : workspace_width, type : "number" });
...
export function workspaceHeightVo( workspace_height : CardProps["workspace_height"] ) : number {
  const name : string = "workspace_height";

  if ( workspace_height !== 0 ) baseVo({ name, value : workspace_height, type : "number" });
