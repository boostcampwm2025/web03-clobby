import { NotAllowMaxLengthText, NotAllowMimeType, NotAllowMinValue } from "@error/domain/user/user.error";
...
// key_name에 대한 검증
export function cardItemAssetKeyNameVo( key_name : CardItemAssetProps["key_name"] ) : string {
  ...
  if ( key_name.length > length ) throw new NotAllowMaxLengthText({ name, length });
...
export function cardItemAssetMimeTypeVo( mime_type : CardItemAssetProps["mime_type"] ) : string {
...
export function cardItemAssetStatusVo( status : CardItemAssetProps["status"] ) : CardItemAssetStatusProps {
