...
export function passwordHashVo( hash : Exclude<UserProps["password_hash"], undefined> ) : string | never {
...
const providerTypes : Array<string> = [
  "kakao"
];
...
export function memeTypeVo( meme_type : UserProfileProps["meme_type"] ) {
  ...
  if ( !memeTypeList.includes(meme_type) ) throw new NotAllowMemeType(memeTypeList);
