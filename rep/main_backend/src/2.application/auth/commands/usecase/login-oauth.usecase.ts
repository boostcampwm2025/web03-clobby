...
export class LoginOauthUsecase<T1, T2> {
  ...
  private readonly selectUserAndOauthWhereEmailFromDb : LoginOauthUsecaseProps<T1, T2>["selectUserAndOauthWhereEmailFromDb"];
  private readonly tokenIssuersInterfaceMakeIssuer : LoginOauthUsecaseProps<T1, T2>["tokenIssuersInterfaceMakeIssuer"];
  private readonly makeHash : LoginOauthUsecaseProps<T1, T2>["makeHash"];
  private readonly insertRefreshDataToCache : LoginOauthUsecaseProps<T1, T2>["insertRefreshDataToCache"];

  constructor ({
    usecaseValues, selectUserAndOauthWhereEmailFromDb, tokenIssuersInterfaceMakeIssuer, makeHash, insertRefreshDataToCache
  ...
  public async execute( dto : LoginOauthUserDto ) : Promise<TokenDto> {
    ...
    // 1. email 검증
    const oauthUser : CheckOauthDataType | undefined = await this.selectUserAndOauthWhereEmailFromDb.select({
    ...
    if ( oauthUser.provider !== dto.provider || oauthUser.provider_id !== dto.provider_id ) throw new NotAllowOauthLoginError();
