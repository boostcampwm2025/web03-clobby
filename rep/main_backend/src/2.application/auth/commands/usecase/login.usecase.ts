...
export class LoginUsecase<T1, T2> {
  ...
  constructor ({
    usecaseValues, selectUserEmailFromDb, compareHash, tokenIssuersInterfaceMakeIssuer, makeHash, insertRefreshDataToCache
  ...
  public async execute( dto : LoginNormalUserDto ) : Promise<TokenDto> {
    ...
    const passwordChecked : boolean = await this.compareHash.compare({ value : dto.password, hash : user.password_hash || "" });
