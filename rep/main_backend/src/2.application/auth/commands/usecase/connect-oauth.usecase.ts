...
import { 
  type CreateUserOauthDto 
} from "@app/auth/commands/dto";
import { 
  SelectDataFromDb
} from "@app/ports/db/db.inbound";
import { 
  InsertValueToDb 
} from "@app/ports/db/db.outbound";
import { 
  OauthUserProps,
  UserProps 
...
import { 
  UserAggregate 
} from "@domain/user/user.aggregate";
import { 
  IdGenerator 
} from "@domain/shared";
...
export class SignUpOauthUsecase<T> {
  ...
  constructor({
    usecaseValues, selectDataWhereEmailFromDb, userIdGenerator, insertUserAndOauthDataToDb
  ...
  public async execute( dto : CreateUserOauthDto ) : Promise<boolean> {
    ...
    const oauthData : OauthUserProps | undefined = userAggregate.connectOauth( {
