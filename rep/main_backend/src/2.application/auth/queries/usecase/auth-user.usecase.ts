...
export class JwtAuth<T> {
  ...
  constructor({
    usecaseValues, accessTokenIssuer, refreshTokenVerify, selectRefreshTokenHashFromCache, compareHash 
  } : JwtAuthProps<T> ) {
    ...
    try {
      ...
      if ( err.code === "TOKEN_EXPIRED" ) {
        try {
          ...
          const namespace : string = `${this.usecaseValues.sessionNameSpace}:${payload.user_id}`.trim();
          const refresh_token_hash : string = await this.selectRefreshTokenHashFromCache.select( { namespace, keyName : this.usecaseValues.refreshTokenHashKeyName } );
          if ( !refresh_token_hash ) throw new UnthorizedError("refresh_token이 존재하지 않습니다.");

          // 2-3. redis에서 refresh_token_hash검증하기
          const tokenChecked : boolean = await this.compareHash.compare({ value : dto.refresh_token, hash : refresh_token_hash });
          if ( !tokenChecked ) throw new UnthorizedError("refresh_token을 다시 확인해주시길 바랍니다.");
