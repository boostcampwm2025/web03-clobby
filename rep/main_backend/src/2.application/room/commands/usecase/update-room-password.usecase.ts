...
export class UdpateRoomPasswordUsecase<T, CT> {
  ...
  private readonly selectUserInfoInRoomFromDb : UpdateRoomPasswordUsecaseProps<T, CT>["selectUserInfoInRoomFromDb"];
  private readonly hashPassword : UpdateRoomPasswordUsecaseProps<T, CT>["hashPassword"];
  private readonly updateRoomPasswordToDb : UpdateRoomPasswordUsecaseProps<T, CT>["updateRoomPasswordToDb"];
  private readonly updateRoomPasswordToCache : UpdateRoomPasswordUsecaseProps<T, CT>["updateRoomPasswordToCache"];
  ...
  async execute(dto : UpdateRoomPasswordDto) : Promise<void> {
    ...
    // 4. 비밀번호를 cache에 저장한다. 
    const cacheInserted : boolean = await this.updateRoomPasswordToCache.updateKey({ namespace : roomInfo.room_id, keyName : "", updateValue : password_hash });
    if ( !cacheInserted ) {
      await this.updateRoomPasswordToDb.update({ uniqueValue : roomInfo.room_id, updateColName : dto.user_id, updateValue : roomInfo.prev_password }); // 다시 되돌린다.
      await this.updateRoomPasswordToCache.updateKey({ namespace : roomInfo.room_id, keyName : "", updateValue : roomInfo.prev_password }); // 다시 돌려야 한다.
