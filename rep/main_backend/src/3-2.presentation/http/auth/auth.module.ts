...
import { 
  JwtAuth, 
} from "@app/auth/queries/usecase";
import { 
  SelectHsetDataFromRedis 
} from "@infra/cache/redis/user/user.inbound";
    ...
    },
    {
      ...
      useFactory : (
        ...
        return new SignUpUsecase({
          usecaseValues : { emailAttributeName }, selectDataWhereEmailFromDb, insertUserDataToDb, userIdGenerator, makeHash
        })
      },
      inject : [
        USERS_EMAIL_ATTR,
    ...
    // 회원 가입 유스케이스 - oauth
    {
      ...
      useFactory : (
        ...
        return new SignUpOauthUsecase({
          usecaseValues : { emailAttributeName }, selectDataWhereEmailFromDb, userIdGenerator, insertUserAndOauthDataToDb
        })
      },
      inject : [
        USERS_EMAIL_ATTR,
    ...
    // 로그인 유스케이스 - local
    {
      ...
      useFactory : (
        ...
        return new LoginUsecase({
          usecaseValues : { emailAttributeName }, selectUserEmailFromDb, compareHash, tokenIssuersInterfaceMakeIssuer,
          makeHash, insertRefreshDataToCache
        })
      }, 
      inject : [
        USERS_EMAIL_ATTR,
    ...
    // 로그인 유스케이스 - oauth
    {
      ...
      useFactory : (
        ...
        return new LoginOauthUsecase({
          ...
          selectUserAndOauthWhereEmailFromDb, tokenIssuersInterfaceMakeIssuer,
          makeHash, insertRefreshDataToCache
        })
      },
      inject : [
    ...
    // 로그아웃 usecase
    {
      ...
      useFactory : (
        ...
        return new LogoutUseCase({
          usecaseValues : { userSessionNamespace, refreshTokenHashKeyName }, deleteUserSessionToCache
        })
      },
      inject : [
        JWT_SESSION_NAMESPACE_ATTR,
