...
import { DB_OAUTH_USERS_ATTRIBUTE_NAME, DB_TABLE_NAME, DB_USERS_ATTRIBUTE_NAME, MYSQL_DB } from "@infra/db/db.constants";
...
export class InsertOauthAndUserDataToMysql extends InsertValueToDb<Pool> {
  ...
  private async insertOauthAndUserData({
    ...
    db : Pool, entity: InsertOauthUserDataProps
  }) : Promise<boolean> {
    ...
    try {
      ...
      if ( userInsertChecked && userInsertChecked.affectedRows && oauthUserInsertChecked && oauthUserInsertChecked.affectedRows ) return true;
      else return false;
    } catch (err) {
...
export class InsertUserDataToMySql extends InsertValueToDb<Pool> {
  ...
  public async insertUserData({
    ...
    db : Pool, tableName : string, entity : UserProps
  }) : Promise<boolean> {
    ...
    try {
      ...
      const [ users ] = await connect.query<ResultSetHeader>(sql, [ entity.user_id, entity.email, entity.nickname, entity.password_hash ]);
  ...
  public async insert(entity: UserProps): Promise<boolean> {
    ...
    const inserChecked : boolean = await this.insertUserData({ db, tableName, entity });
