...
import { CACHE_USER_NAMESPACE_NAME, CACHE_USER_SESSION_KEY_NAME, REDIS_SERVER } from "../../cache.constants";
...
export class InsertUserSessionDataToRedis extends InsertDataToCache<RedisClientType<any, any>> {
  ...
  private async insertUserSession({
    cache, np, refresh_token_hash
  } : {
    cache : RedisClientType<any, any>, np : string, refresh_token_hash : string
  }) : Promise<boolean> {
  ...
  public async insert(entity: InsertCacheDataProps): Promise<boolean> {
    ...
    const insertChecked : boolean = await this.insertUserSession({ cache, np, refresh_token_hash : entity.refresh_token_hash });
...
export class DeleteUserDataToRedis extends DeleteDataToCache<RedisClientType<any, any>> {
  ...
  // keyname 삭제
  public async deleteKey({ namespace, keyName }: { namespace: string; keyName: string; }): Promise<boolean> {
