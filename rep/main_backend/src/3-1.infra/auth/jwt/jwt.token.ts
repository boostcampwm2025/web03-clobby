...
export class JwtTokenIssuer extends TokenIssuer {
  ...
  private async makeJwt({
    payload, secret_key, expired_time} : {
    payload : Payload, secret_key : string, expired_time : string
  }) : Promise<string> {
  ...
  public async makeIssuer(payload: Payload): Promise<TokenDto> {
    ...
    const access_token : string = await this.makeJwt({ 
      payload, secret_key : secretKeys.access_token_secret, expired_time : expiredTimes.access_token_expired 
    });
    
    const refresh_token : string = await this.makeJwt({
      payload, secret_key : secretKeys.refresh_token_secret, expired_time : expiredTimes.refresh_token_expired
    });
