...
describe("domain 레이어에 유저 entity 관련 단위테스트", () => {
  ...
  const TestCase1 : Array<TestCase1Props> = [
    { user_id : "018e9a48-3c8a-7b20-bc9d-9f5b8e9f88c0", 
      ...
      password_hash : "$argon2id$v=19$m=65536,t=3,p=4$YWJjZGVmZ2hpamtsbW5vcA$w7ZxJwLJqz0mH9g7t6Hl8AnmVhZ+eakTnXh/J0Kv2g8", 
      nickname : "완두콩", 
      created_at : now, 
      updated_at : now,
      ok : true  },
  ...
  test.each(TestCase1)("user entity에 정합성을 파악하는 단위테스트", ({ user_id, email, password_hash, nickname, created_at, updated_at, ok }) => {
    const test1 = () => {
      user_id = user_id as string;
      email = email as string;
      nickname = nickname as string;
      return new User({
        user_id, email, password_hash, nickname, created_at, updated_at
      });
    }
    if ( ok ) {
      const t1 = test1();
  ...
  test.each(TestCase2)("oauth entity에 정합성을 파악하는 단위테스트", ({ id, user_id, provider, provider_id, created_at, updated_at, ok }) => {
    const test2 = () => {
      id = id as number;
      user_id = user_id as string;
      ...
      return new OauthUser({ id, user_id, provider, provider_id, created_at, updated_at });
    };
  ...
  test.each(TestCase3)("user profile entity가 정합성을 제대로 지키는지 확인", ({ id, user_id, profile_path, meme_type, created_at, updated_at, ok }) => {
    const test3 = () => {
      id = id as number;
      user_id = user_id as string;
      ...
      return new UserProfile({
        id, user_id, profile_path, meme_type
      });
    };
